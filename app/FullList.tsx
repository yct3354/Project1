...
export default function FullList({ route }: any) {
  ...
  const [filterState, setFilterState] = useState<number[]>([0, 0, 0, 0, 0, 0]);
  ...
  const filterString = () => {
    ...
    if (filterState[5] != 0) {
      directionString =
        " AND direction" + (filterState[5] === 1 ? " = " : " <> ") + "-1";
    }
    return (
      dateString +
      hiddenString +
  ...
  const fetchInitData = async () => {
    try {
      ...
      if (mode != "All") {
        let arr = [...filterState];
        arr[2] = fxList.findIndex((item: any) => item.currency === mode) + 1;
        setFilterState(arr);
      } else {
        ...
        >
          <View style={{ width: "100%", height: "35%" }}>
            ...
            <FilterButton
              ...
              fxFilterInit={fxTags.indexOf(mode) + 1}
            />
          </View>
